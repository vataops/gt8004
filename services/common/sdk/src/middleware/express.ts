...
export interface MiddlewareOptions {
  ...
  /** Capture request/response body content. Default: true */
  captureBody?: boolean;
...
export function createExpressMiddleware(
  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    res.end = function (this: Response, ...args: any[]): Response {
      ...
      if (clientIp) headers['client-ip'] = clientIp;
      if (req.headers['x-agent-id']) headers['x-agent-id'] = req.headers['x-agent-id'] as string;
      if (acceptLanguage) headers['accept-language'] = acceptLanguage;

      const entry: RequestLogEntry = {
        requestId,
        customerId: options.extractCustomerId?.(req) ?? clientIp,
