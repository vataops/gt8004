...
import (
	...
	"github.com/GT8004/gt8004-analytics/internal/geoip"
...
func NewEnricher(s *store.Store, logger *zap.Logger, maxBodySize int, geo *geoip.Resolver) *Enricher {
	if maxBodySize <= 0 {
...
func (e *Enricher) Process(ctx context.Context, agentDBID uuid.UUID, batch *LogBatch) error {
	...
	// Determine source from SDK version: gateway-* -> "gateway", else -> "sdk"
	sourceStr := "sdk"
	...
	for i, entry := range batch.Entries {
		// Use entry-level source if provided, otherwise batch-level
		...
		// Resolve GeoIP if IP is present and country/city not already set
		if e.geo != nil && logs[i].IPAddress != nil && logs[i].Country == nil {
			if result := e.geo.Lookup(*logs[i].IPAddress); result != nil {
				if result.Country != "" {
					logs[i].Country = &result.Country
				}
				if result.City != "" {
					logs[i].City = &result.City
				}
			}
		}
		...
		// Aggregate per-customer stats keyed by IP address
