...
// Job periodically cleans up old request/response body data.
type Job struct {
	store         *store.Store
	retentionDays int
...
// NewJob creates a new body data retention cleanup job.
func NewJob(s *store.Store, retentionDays int, logger *zap.Logger) *Job {
	return &Job{
		store:         s,
