...
func (s *Store) UpsertCustomer(ctx context.Context, agentDBID uuid.UUID, customerID string, requestCount int64, revenue float64, avgMs float32, errorRate float32) error {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO customers (agent_id, customer_id, total_requests, total_revenue, avg_response_ms, error_rate, last_seen_at)
		...
		ON CONFLICT (agent_id, customer_id) DO UPDATE SET
			...
			last_seen_at    = NOW(),
			updated_at      = NOW()
	`, agentDBID, customerID, requestCount, revenue, avgMs, errorRate)
	if err != nil {
