...
def build_agent_uri() -> str:
    ...
    metadata = {
        ...
        "name": os.environ.get("AGENT_NAME", "Calc Agent"),
        "description": os.environ.get(
            "AGENT_DESCRIPTION",
            "Math utility agent supporting expression evaluation, unit conversion, random number generation, and statistics via A2A and MCP protocols",
        ),
        ...
        "services": [
            {
                "name": "a2a",
                "endpoint": agent_url,
                "skills": ["calculate", "unit-convert", "random", "statistics"],
            },
            {
                "name": "mcp",
                "endpoint": agent_url + "/mcp" if agent_url else "",
                "mcpTools": ["calculate", "unit_convert", "random_number", "statistics"],
            },
        ],
        "active": True,
    }
...
def main():
    ...
    print(f"Name:     {os.environ.get('AGENT_NAME', 'Calc Agent')}")
