...
def build_agent_uri() -> str:
    ...
    metadata = {
        ...
        "name": os.environ.get("AGENT_NAME", "Toolbox Agent"),
        "description": os.environ.get(
            "AGENT_DESCRIPTION",
            "Developer utility agent providing JSON formatting, timestamp conversion, regex testing, and text diffing via A2A and MCP protocols",
        ),
        ...
        "services": [
            {
                "name": "a2a",
                "endpoint": agent_url,
                "skills": ["json-format", "timestamp", "regex-test", "diff-text"],
            },
            {
                "name": "mcp",
                "endpoint": agent_url + "/mcp" if agent_url else "",
                "mcpTools": ["json_format", "timestamp_convert", "regex_test", "diff_text"],
            },
        ],
        "active": True,
    }
...
def main():
    ...
    print(f"Name:     {os.environ.get('AGENT_NAME', 'Toolbox Agent')}")
