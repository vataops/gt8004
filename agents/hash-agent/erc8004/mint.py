...
Environment (.env):
    ...
    AGENT_URL             - Deployed agent URL (for services metadata)
    BASE_SEPOLIA_RPC      - RPC endpoint
    IDENTITY_REGISTRY     - Contract address

Note: Endpoint is included in on-chain metadata under services.
"""
...
def build_agent_uri() -> str:
    """Build a data: URI with agent identity metadata including services."""
    agent_url = os.environ.get("AGENT_URL", "")
    metadata = {
        "type": "https://eips.ethereum.org/EIPS/eip-8004#registration-v1",
        "name": os.environ.get("AGENT_NAME", "Hash Agent"),
        "description": os.environ.get(
            "AGENT_DESCRIPTION",
            "Cryptographic utility agent — hashing, encoding, checksumming, UUID generation via A2A and MCP",
        ),
        ...
        "services": [
            {
                "name": "a2a",
                "endpoint": agent_url,
                "skills": ["hash", "encode", "checksum", "uuid"],
            },
            {
                "name": "mcp",
                "endpoint": agent_url + "/mcp" if agent_url else "",
                "mcpTools": ["hash", "encode", "checksum", "uuid_gen"],
            },
        ],
        "active": True,
    }
...
def main():
    ...
    print(f"Name:     {os.environ.get('AGENT_NAME', 'Hash Agent')}")
