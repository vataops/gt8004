...
mcp = FastMCP("wallet-profiler-mcp")
...
@mcp.tool()
async def about() -> str:
    """Learn what the Wallet Profiler Agent does — wallet behavior profiling, portfolio analysis, risk assessment, and activity patterns."""
    return (
        f"{settings.agent_name}\n\n"
        f"{settings.agent_description}\n\n"
        "Capabilities: Wallet profiling, portfolio composition, risk scoring, activity analysis\n\n"
        "Available tools:\n"
        "- profile(address): Comprehensive wallet behavior profiling\n"
        "- portfolio(address): Portfolio composition and diversification analysis\n"
        "- risk(address): Wallet risk scoring and assessment\n"
        "- activity(address): On-chain activity pattern analysis\n\n"
        "Payment: x402 protocol, $0.01 USDC on Base per tool call"
    )


@mcp.tool()
async def profile(address: str) -> str:
    """Profile an Ethereum/EVM wallet address — classify behavior (whale, smart money, DeFi farmer, NFT collector), identify activity patterns, and generate a behavioral fingerprint. Example: profile('0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045')"""
    return await _call_a2a(
        f"Provide a comprehensive behavioral profile of the wallet address: {address}",
        skill_id="profile",
    )


@mcp.tool()
async def portfolio(address: str) -> str:
    """Analyze the portfolio composition of a wallet — asset allocation, token holdings, DeFi positions, diversification score, and cross-chain distribution. Example: portfolio('0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045')"""
    return await _call_a2a(
        f"Analyze the portfolio composition of the wallet address: {address}",
        skill_id="portfolio",
    )


@mcp.tool()
async def risk(address: str) -> str:
    """Assess the risk profile of a wallet — smart contract interaction risk, approval hygiene, counterparty exposure, rugpull exposure, and compliance flags. Example: risk('0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045')"""
    return await _call_a2a(
        f"Provide a risk assessment of the wallet address: {address}",
        skill_id="risk",
    )


@mcp.tool()
async def activity(address: str) -> str:
    """Analyze on-chain activity patterns of a wallet — transaction frequency, DeFi interactions, bridge usage, governance participation, and temporal patterns. Example: activity('0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045')"""
    return await _call_a2a(
        f"Analyze the on-chain activity patterns of the wallet address: {address}",
        skill_id="activity",
    )
