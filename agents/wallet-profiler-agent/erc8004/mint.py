"""
ERC-8004 Token Minting Script for Base Mainnet
===============================================
Mints a new agent identity token on the ERC-8004 Identity Registry (Base chain).
...
Environment (.env):
    ...
    ETH_RPC               - Base mainnet RPC endpoint
...
def build_agent_uri() -> str:
    ...
    metadata = {
        ...
        "name": os.environ.get("AGENT_NAME", "Stablecoin Search Agent"),
...
def main():
    ...
    print(f"Chain:    Base Mainnet ({CHAIN_ID})")
    print(f"Registry: {registry_addr}")
    print(f"Name:     {os.environ.get('AGENT_NAME', 'Stablecoin Search Agent')}")
    print(f"Version:  {os.environ.get('AGENT_VERSION', '1.0.0')}")
    print(f"x402:     Enabled (0.00001 ETH/request)")
