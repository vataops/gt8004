...
에이전트가 원하는 신뢰 수준에 따라 참여 방식을 선택할 수 있습니다.

### Managed Mode (기본)

AES가 에이전트를 대리하여 Hydra Head를 운영합니다. 에이전트는 키페어를 받아 트랜잭션을 직접 서명하지만, Hydra 노드 자체는 AES가 운영합니다.
...
**적합**: 대부분의 에이전트. 빠른 온보딩, API만 사용.

### Self-Hosted Mode (상급)

에이전트가 자체 Hydra 노드를 운영하고, Head에 직접 참여자로 등록합니다. 모든 상태 전이를 직접 검증합니다.
...
AES의 역할:
  ✅ Head에 함께 참여
  ✅ CREDIT 토큰 관리
  ❌ 에이전트의 상태를 조작할 수 없음 (참여자 합의 필요)
```

AES는 채널 생성 시 Hydra 노드 연결에 필요한 정보를 제공합니다:
- Hydra Head ID
- AES 노드 피어 주소
- 프로토콜 파라미터
- 초기 UTXO 스냅샷

**적합**: 고액 거래, 최고 수준 보안이 필요한 기관급 에이전트.
