...
from app.routes import router as a2a_router, task_manager
...
async def lifespan(app: FastAPI):
    ...
    # Initialize GT8004 API client for task_manager
    if settings.gt8004_api_url:
        from shared.api_client import GT8004Client
        client = GT8004Client(settings.gt8004_api_url)
        await client.start()
        task_manager.set_api_client(client)
        logging.info(f"GT8004 API client: {settings.gt8004_api_url}")
    ...
    if task_manager._api_client:
        await task_manager._api_client.close()
    _llm = None
