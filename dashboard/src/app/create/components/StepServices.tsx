...
const SERVICE_TYPES: { key: ServiceEntry["type"]; label: string; color: string; description: string }[] = [
  ...
  { key: "oasf", label: "OASF", color: "text-purple-400 border-purple-600 bg-purple-900/20", description: "Open Agentic Schema Framework service" },
  { key: "custom", label: "Custom", color: "text-gray-400 border-gray-600 bg-gray-900/20", description: "Custom service type" },
];
    ...
    <div>
      ...
      {services.length > 0 && (
        <div className="mb-5 space-y-2">
          <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Added Services ({services.length})</p>
          {services.map((svc, i) => {
            const typeInfo = SERVICE_TYPES.find((t) => t.key === svc.type)!;
            return (
              <div key={i} className="flex items-center justify-between p-3 rounded-lg border border-gray-700 bg-gray-900/50">
                <div className="min-w-0 flex-1">
                  ...
                  {svc.mcpTools.length > 0 && (
                    <p className="text-xs text-gray-500">Tools: {svc.mcpTools.join(", ")}</p>
                  )}
                </div>
                <button
      ...
      {activeTab === "mcp" && (
        <>
          <div className="mb-3">
            <label className="block text-xs text-gray-500 mb-1">Tools (comma-separated, optional)</label>
            <input
              type="text"
              value={toolsInput}
          ...
          <div className="mb-3">
            <label className="block text-xs text-gray-500 mb-1">Prompts (comma-separated, optional)</label>
            <input
              type="text"
              value={promptsInput}
          ...
          <div className="mb-3">
            <label className="block text-xs text-gray-500 mb-1">Resources (comma-separated, optional)</label>
            <input
              type="text"
      ...
      <button
        ...
        className="w-full py-2.5 rounded-md border border-gray-700 text-sm text-gray-300 hover:bg-gray-800 hover:text-white disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
      >
